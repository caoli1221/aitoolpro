document.addEventListener('DOMContentLoaded', async () => {
  ...
  (function() {
    if (localStorage.getItem('aitool_dark') === '1') {
      document.getElementById('bodyEl').classList.add('dark');
  ...
  window.sortTools = function() {
    ...
    if (val === 'rating') cards.sort((a,b) => parseFloat(b.dataset.rating) - parseFloat(a.dataset.rating));
    else if (val === 'name') cards.sort((a,b) => a.dataset.name.localeCompare(b.dataset.name));
    else if (val === 'category') cards.sort((a,b) => a.dataset.category.localeCompare(b.dataset.category));
  ...
  window.toggleView = function() {
    ...
    const grid = document.getElementById('featuredGrid').parentElement;
    const btn = document.getElementById('viewToggle');
    if (isListView) {
      grid.classList.add('list-view');
      btn.textContent = '🔲';
      document.querySelectorAll('.tool-card .tool-head').forEach(h => {
        const inr = h.parentElement;
        const iconDiv = h.querySelector('.tool-icon');
        const rest = document.createElement('div'); rest.className = 'flex items-center gap-2';
        rest.innerHTML = h.querySelector('.fav-btn').outerHTML + h.querySelector('.fav-btn').nextElementSibling?.outerHTML || '';
        h.replaceWith(iconDiv);
        inr.insertBefore(rest, inr.querySelector('.tool-body'));
      });
    } else {
  ...
  window.toggleFav = function(toolId) {
    ...
    if (idx > -1) favs.splice(idx, 1); else favs.push(toolId);
    localStorage.setItem('aitool_favs', JSON.stringify(favs));
    // Update star opacity
    document.querySelectorAll('.fav-btn').forEach(btn => {
      const card = btn.closest('.tool-card');
      if (card && card.dataset.id === toolId) {
        btn.style.opacity = idx > -1 ? '0.3' : '1';
        btn.title = idx > -1 ? '加入收藏' : '取消收藏';
      }
    });
    renderFavBar();
  };
  ...
  window.renderFavBar = function() {
    ...
    list.innerHTML = favs.map(id => {
      ...
      if (!t) return '';
      return '<a href="pages/tool.html?id='+id+'" class="flex-shrink-0 bg-white dark:bg-gray-800 rounded-lg px-3 py-2 text-sm border border-gray-200 dark:border-gray-700 hover:border-purple-400 dark:hover:border-purple-500 transition whitespace-nowrap">'+t.icon+' '+t.name+'</a>';
    }).join('');
  };
  ...
  window.renderRecentBar = function() {
    ...
    list.innerHTML = recent.map(id => {
      const t = tools.find(x => x.id === id);
      if (!t) return '';
      return '<a href="pages/tool.html?id='+id+'" class="flex-shrink-0 bg-white dark:bg-gray-800 rounded-lg px-3 py-2 text-sm border border-gray-200 dark:border-gray-700 hover:border-blue-400 dark:hover:border-blue-500 transition whitespace-nowrap">'+t.icon+' '+t.name+'</a>';
    }).join('');
  };
  ...
  // === SEARCH DARK MODE ===
  const origSearch = window.handleSearch;
